...
import { bridge, type GitStatus } from "../core/bridge";
...
export default function ChangesPanel() {
  ...
  if (liveSession) return <LiveChanges />;
  return <MockChanges />;
}

/** Uncommitted changes from Core's `git_status` (G3). Ownership needs a running
 *  task's change ledger (G8), so with no task this is a plain status list. */
function LiveChanges() {
  ...
  if (error) {
    return <ErrorState presentation={present(error, { context: "Reading changes" })} />;
  }
  if (!status) {
    return (
      <div style={{ padding: 12, fontSize: "var(--text-sm)", color: "var(--text-tertiary)" }}>
        Reading git status…
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "0 12px 8px" }}>
        <span className="section-label">
          ...
          {status.branch ? ` · ${status.branch}` : status.detached ? " · (detached)" : ""}
        </span>
      </div>
      <div className="scroll-y" style={{ flex: 1 }}>
        ...
        {files.map((f) => (
          ...
          >
            ...
            <span style={{ fontSize: 10, color: "var(--text-tertiary)" }}>{f.staged ? "staged · " : ""}{f.kind}</span>
          </button>
        ))}
      </div>
    </div>
