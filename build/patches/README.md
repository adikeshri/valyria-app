...
| `032-valyria-letterpress-watermark.patch` | `src/vs/workbench/browser/parts/editor/media/letterpress-{dark,light,hcDark,hcLight}.svg` | The empty‑editor‑group watermark (`editorGroupWatermark.ts`'s `.letterpress`, referenced via CSS `background-image: url(...)`, so it can't be recoloured or swapped from `extension/` or a theme) is stock Code's generic sidebar‑and‑lines glyph. Replaced with a vector trace of the dragon head from the brand mark (`docs/assets/logo.png`) — same `<path>`‑only, no‑explicit‑fill / fixed‑HC‑grey structure as the originals, so the existing opacity and high‑contrast handling in `editorgroupview.css` needs no change. |
