...
The Rust protocol client that sits between the Tauri host and a Core
(`valyria serve`) daemon. It has **no Tauri dependency** and depends only on
`valyria-protocol` + `valyria-types` from Core — `xtask check-layering` fails
if that ever stops being true, so the wire contract stays the single coupling
point.
