...
**Phase 2 (in progress)**: `apps/desktop` is now an npm-workspace member and
consumes `@valyria/protocol` + `@valyria/state`. The renderer side of the spine
is wired: `src/core/bridge.ts` (typed `invoke`/`listen` over the host commands),
`src/core/liveStore.ts` (a zustand store folding `core://event-batch` through
the pure reducer), and `ConnectBar` (point the app at a workspace root → spawn /
adopt). The reducer/store/selectors grew task-objective, plan and approval
projections plus an `activityLine` narrative (no model-reasoning text — §10).
The **Timeline panel** and the **header connection/runtime indicator** now render
live when a session is open and fall back to `mock.ts` (labeled "sample data")
otherwise. `packages/state/test/replay.test.ts` (node:test + tsx) replays a real
captured trace (`fixtures/traces/add-a-function.jsonl`, generated by the
`#[ignore] capture_trace` bridge helper) and asserts the projection is gapless,
idempotent on resume, and hole-detecting.

**Phase 3 (in progress) — repository surfaces via the local-read exception**
(CORE-INTERFACE §3). `valyria-bridge` gained `workspace_fs` (list dir / read
file, **scoped to the authorized root** — `resolve` rejects absolute paths,
`..`, and symlink escape via `canonicalize` + containment re-check; unit-tested
including the escape cases) and `git` (status / diff / log / branch by shelling
out — read-only; writes stay Core's, §17). The host exposes `fs_list_dir` /
`fs_read_file` / `git_status` / `git_log` / `git_diff` / `git_branch`
(`spawn_blocking`, off the async runtime). Renderer: `src/core/repo.ts`
wrappers, a `ServedLocally` marker on every fallback surface, and live-or-mock
`FileTreePanel` (lazy per-dir load + git-status dots), `GitPanel` (read-only,
write button disabled with the G3 reason), and `CodeViewerPanel`
(`fs_read_file`, binary + truncation handling). Layering check still green — the
new modules use only `std` + `serde`.

Still to do in Phase 3: an fs-watcher → `core://fs-changed` for the "external
changes within 500ms" exit criterion, virtualization for very large directories,
and the local search UI behind the capability switch.

Still on `mock.ts`, to migrate after: Agent chat / task creation, Activity feed,
Task panel + plan view, pause/resume/cancel wiring, task history, the
resume-on-launch prompt, and tightening the per-kind payload decoders against
the trace corpus with the coverage gate.
