...
const MODELS: Record<string, unknown> = {
  ...
  modelChat: {
    ...
    canSubmit: false,
    transcript: [
      { seq: 1, role: "you", text: "Add a --json flag to the export command" },
      { seq: 2, role: "agent", text: "Editing src/cli.rs" },
    ],
    approval: {
      seq: 3,
  ...
  home: {
    ...
    active: [{ id: "t1", objective: "Add a flag", state: "implementing", terminal: false, blocked: false, filesTouched: 2, when: "2026-09-03" }],
    recent: [{ id: "t0", objective: "Fix the bug", state: "completed", terminal: true, blocked: false, filesTouched: 1, when: "2026-09-02" }],
  },
  workspace: {
    connection: "ready", taskId: "t1", objective: "Add a --json flag", state: "implementing", terminal: false,
    working: true, blocked: false, canSubmit: false,
    transcript: [{ seq: 1, role: "you", text: "Add a --json flag" }, { seq: 2, role: "agent", text: "Editing src/x.rs" }],
    planSteps: [{ intent: "wire the flag", status: "done", checkpoint: true }, { intent: "cover with a test", status: "active", checkpoint: false }],
    ...
    verified: [{ kind: "test", command: "cargo test", outcome: "Pass" }], unverified: ["lint"],
    approval: { seq: 3, prompt: "Run cargo test?", tool: "run_command", risk: "standard" },
  },
  review: {
    connection: "ready", taskId: "t1", objective: "Add a --json flag", ledgerAvailable: true, reportStatus: "completed",
    files: [
      { path: "src/x.rs", change: "modified", ownership: "agent_authored" },
      { path: "src/y.rs", change: "modified", ownership: "concurrent_user_modification" },
    ],
    verified: [{ kind: "test", command: "cargo test", outcome: "Pass" }], unverified: ["lint"],
    approval: null,
  },
